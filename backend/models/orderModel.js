...
const OrderModel = {
  create: async ({ order_number, buyer_id, seller_id, service_id, total_amount,
                   platform_fee, payment_method, address, lat, lng, scheduled_at, notes }) => {
    const [result] = await pool.query(
      `INSERT INTO orders
       ...
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [order_number, buyer_id, seller_id, service_id || null, total_amount,
       platform_fee || 0, payment_method || "cash", address, lat || null, lng || null,
  ...
  updateStatus: async (id, status, extra = {}) => {
    const extras = Object.keys(extra).map((k) => `, ${k} = ?`).join("");
