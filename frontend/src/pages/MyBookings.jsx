      ...
      <div className="mx-auto max-w-4xl relative z-10">
        <div className="mb-8 flex flex-col md:flex-row md:items-end md:justify-between gap-6 border-b border-slate-200 pb-6">
          <div>
            ...
            <h1 className="text-2xl md:text-3xl font-bold text-slate-800 mt-1">My Bookings</h1>
          </div>
        ...
        {loading ? (
          ...
          <div className="rounded-2xl border border-slate-200 bg-white p-16 text-center shadow-sm flex flex-col items-center justify-center">
            ...
            <h2 className="text-xl md:text-2xl font-bold text-slate-850">No bookings yet</h2>
          ...
          <div className="space-y-6">
            {sortedBookings.map((booking) => {
              ...
              return (
                <article
                  key={booking.id}
                  className="rounded-2xl border border-slate-200 bg-white p-4 sm:p-6 shadow-xs hover:shadow-md transition-shadow text-left space-y-2.5"
                >
                  {/* Top Header Row: Avatar + Order ID on left, Status badge on right */}
                  <div className="flex items-center justify-between gap-3 pb-2 border-b border-slate-100">
                    <div className="flex items-center gap-2.5 min-w-0">
                      {booking.seller_pic ? (
                        <img
                          ...
                          className="h-10 w-10 rounded-full border border-slate-200 object-cover shadow-2xs shrink-0"
                        />
                      ) : (
                        <div className="h-10 w-10 rounded-full bg-gradient-to-br from-blue-600 to-indigo-600 font-bold text-white flex items-center justify-center text-sm shadow-2xs shrink-0">
                          {avatarLetter}
                        </div>
                      )}
                      <div className="min-w-0">
                        <span className="inline-flex items-center text-xs font-mono text-slate-600 bg-slate-100 px-2 py-0.5 rounded-md border border-slate-200 font-semibold">
                    ...
                    <div className="flex items-center gap-1 shrink-0">
                      ...
                      {booking.scheduled_at && new Date(booking.scheduled_at) < new Date() && ["pending", "accepted", "in_progress", "quoted"].includes(booking.status) && (
                        <span className="rounded-full border border-amber-200 bg-amber-50 px-2 py-0.5 text-[11px] sm:text-xs font-bold uppercase text-amber-700">
                          ⚠️ Overdue
                        </span>
                      )}
                    </div>
                  ...
                  <div className="space-y-1.5 text-xs text-slate-600">
                    <div className="flex flex-wrap items-center justify-between gap-1 bg-blue-50/70 p-2.5 rounded-xl border border-blue-100">
                      <span className="font-semibold text-slate-700">Scheduled Visit:</span>
                      <span className="font-bold text-blue-800 text-[11px] bg-white px-2 py-0.5 rounded-md border border-blue-200/80">
                    ...
                    <div className="flex items-start gap-1 px-1">
                      ...
                      <span className="font-semibold text-slate-800 break-all">{booking.address || "—"}</span>
                    </div>

                    {booking.seller_phone && (
                      <div className="flex items-center gap-1 px-1">
                        <span className="font-semibold text-slate-500 shrink-0">Seller Contact:</span>
                        <a href={`tel:${booking.seller_phone}`} className="font-bold text-blue-600 hover:underline">
                          {booking.seller_phone}
                        </a>
                      </div>
                    )}
