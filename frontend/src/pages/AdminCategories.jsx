    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        ...
        <button
          ...
          className="px-4 py-2 bg-indigo-600 hover:bg-indigo-500 text-white font-bold rounded-xl text-xs flex items-center gap-1.5 transition-all shadow-lg shadow-indigo-950/40 self-start"
        >
          <PlusCircle size={14} />
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {loading ? (
          <div className="col-span-full py-12 text-center text-slate-500 text-xs animate-pulse font-medium bg-slate-900/40 rounded-2xl border border-indigo-950/40">
            Fetching service categories...
          </div>
        ) : categories.length === 0 ? (
          <div className="col-span-full py-12 text-center text-slate-500 text-xs font-medium bg-slate-900/40 rounded-2xl border border-indigo-950/40">
          ...
          categories.map((cat) => (
            <div
              key={cat.id}
              className="bg-slate-900/40 border border-indigo-950/40 rounded-2xl p-6 backdrop-blur-md flex flex-col justify-between space-y-4 hover:border-indigo-500/30 transition-all duration-300"
            >
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <span className="text-3xl p-2 bg-slate-950/40 rounded-xl border border-indigo-950/60 shadow-inner">
                      {cat.icon}
                    </span>
                    <div>
              ...
              <div className="border-t border-indigo-950/60 pt-4 flex items-center justify-between">
                <span className="text-[10px] text-slate-500 font-extrabold uppercase">Platform Status</span>
                <button
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-xs p-4">
          <div className="bg-slate-900 border border-indigo-900/40 rounded-2xl max-w-md w-full p-6 space-y-5 shadow-2xl animate-fade-in">
            ...
            <form onSubmit={handleCreateCategory} className="space-y-4">
              <div>
                <label className="block text-slate-400 text-xs font-semibold mb-2">Category Name</label>
                <input
                  type="text"
              ...
              <div>
                <label className="block text-slate-400 text-xs font-semibold mb-2">Icon (Emoji representation)</label>
                <input
                  type="text"
                  placeholder="e.g. 🧹, 🔨, 🚗"
                  value={newCatIcon}
                  ...
                  required
                  disabled={addLoading}
                  className="w-20 px-3 py-2.5 bg-slate-950 border border-indigo-900/50 rounded-xl text-white text-center text-lg placeholder-slate-600 focus:outline-none focus:border-indigo-500 font-semibold"
                />
              </div>

              <div>
                <label className="block text-slate-400 text-xs font-semibold mb-2">Short Description</label>
                <textarea
                  placeholder="Enter details on what types of tasks this category covers..."
                  value={newCatDesc}
                  onChange={(e) => setNewCatDesc(e.target.value)}
                  disabled={addLoading}
                  className="w-full px-3 py-2.5 h-24 bg-slate-950 border border-indigo-900/50 rounded-xl text-white placeholder-slate-600 focus:outline-none focus:border-indigo-500 text-xs font-medium leading-relaxed resize-none"
                />
              </div>
