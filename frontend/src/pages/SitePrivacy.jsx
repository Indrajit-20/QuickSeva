...
const PrivacyPolicy = () => {
  ...
  useEffect(() => {
    document.title = "Privacy Policy - QuickSeva";
    const fetchPolicy = async () => {
      try {
        const result = await getPolicy("privacy_policy");
    ...
    <div className="min-h-screen bg-slate-50 text-slate-800 py-16 px-4 sm:px-6 lg:px-8 flex flex-col justify-between">
      <div className="max-w-4xl mx-auto w-full">
        ...
        <div className="bg-white rounded-2xl p-8 sm:p-12 border border-slate-200 shadow-md relative overflow-hidden text-left">
          <div className="absolute top-0 left-0 right-0 h-[3px] bg-blue-655"></div>

          {loading ? (
            <div className="flex flex-col items-center justify-center py-16 space-y-4">
              <div className="w-12 h-12 border-4 border-blue-600 border-t-transparent rounded-full animate-spin"></div>
              <p className="text-blue-655 animate-pulse font-bold">Loading Privacy Policy...</p>
            </div>
          ) : error ? (
            <div className="text-center py-12 space-y-4">
              ...
              <button
                ...
                className="mt-4 px-6 py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-xl transition-colors font-bold cursor-pointer"
              >
                Try Again
              </button>
            </div>
